...
var
  frm_Search_Text : Tfrm_Search_Text;
...
procedure Tfrm_Search_Text.rgSeListSelectionChanged( Sender : TObject );
begin
  ...
  Se_SI.Condition := ReturnSearchCondition( rgSeList.ItemIndex, cConditionsAllowed_Text );
...
//grab indexes that are needed for enabling / disabling
  fIsEmptyIdx := ReturnSearchConditionIdx( coIsEmpty, cConditionsAllowed_Text );
  fIsRegX := ReturnSearchConditionIdx( coRegEx, cConditionsAllowed_Text );
  rgSeList.Caption := crg_MatchType;

end;

procedure Tfrm_Search_Text.FormHide( Sender : TObject );
begin
  rgSeList.ItemIndex := -1;
end;

procedure Tfrm_Search_Text.FormKeyDown( Sender : TObject; var Key : Word; Shift : TShiftState );
begin
...
procedure Tfrm_Search_Text.cbMatchCaseChange( Sender : TObject );
begin
  if fIsLoading then
    exit;
...
procedure Tfrm_Search_Text.UpdateSearchValue;
begin
...
procedure Tfrm_Search_Text.memSearchValueChange( Sender : TObject );
begin
  if fIsLoading then
    exit;
  UpdateSearchValue;
...
procedure Tfrm_Search_Text.EnableControls( Idx : integer );
begin
...
procedure Tfrm_Search_Text.RegisterBoolExpr( BE : TfrmBoolExpr );
begin
  fBE := BE;
end;

procedure Tfrm_Search_Text.SetSe_SI( AValue : TSearchItem );
var
  LabelType : TSearchOperatorType;
begin

  if AValue.SIT <> sitString then
    raise EErrorDevelopment.Create( 'Tfrm_Search_Text.SetSe_SI: String Type expected.' );

  LabelType := AValue.SearchOperatorType;
  if LabelType <> sotNone then
    raise EErrorDevelopment.Create( 'Tfrm_Search_Text.SetSe_SI: Invalid Operator Type.' );
  ...
  lblSEType.caption := format( cusSearchCaption_Text, [ fSe_SI.siDisplayCaption ] );

  cbMatchCase.Checked := fSe_SI.CaseSensitive;

  memSearchValue.Text := fSe_SI.SearchValue;

  rgSeList.ItemIndex := ReturnSearchConditionIdx( fSe_SI.Condition, cConditionsAllowed_Text );
...
procedure Tfrm_Search_Text.Link_SI_BoolExpr( anSI : TSearchItem; BE : TfrmBoolExpr );
begin
  RegisterBoolExpr( BE );
  Se_SI := anSI;
