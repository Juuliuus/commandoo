...
==================================
June 22, 2017 Julius Schoen: 1.6 fixes for i8n bug where multiline hints/etc broken
hopefullly fixed in lazarus 1.8
}
{$mode objfpc}{$H+}
...
uses
  ...
  unitjTranslations, Forms, LazUTF8, dialogs;
...
procedure SetDefaultLang(Lang: string; Dir: string = ''; ForceUpdate: boolean = true ; OverridePOPath : string = '' );
...
begin
  ...
  try
     if OverridePOPath <> '' then
       lcfn := OverridePOPath
     else lcfn := FindLocaleFileName('.po', Lang, Dir);

     if lcfn <> '' then
     begin
       unitjTranslations.TranslateResourceStrings(lcfn);
       ...
       begin
         ...
         unitjTranslations.TranslateUnitResourceStrings('LCLStrConsts', LCLPath);
       end;
